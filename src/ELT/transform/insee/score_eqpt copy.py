...
def calcul_score(territory_code):
    """
    Calcul le score des équipements pour 10000 habitants
    ...
    df_final = pd.concat([pop, pivot_count, pivot_score], axis=1).reset_index()
    ...
    for _, row in df_final.iterrows():
        ...
        for lib_norm in CATEGORY_MAPPING.values():
            ...
            if not (pd.isna(count_val) and pd.isna(score_val)):
                services[lib_norm] = {
